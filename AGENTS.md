...
- Benchmarks: reuse helpers in `dev/benchmarks/bench_helpers.R`. Run `Rscript dev/benchmarks/run_benchmarks_cli.R > bench_results.tsv` (the pre-commit hook under `githooks/` does the same once you `git config core.hooksPath githooks`). Diff the TSV across commits to track perf trends.
