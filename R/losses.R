...
mlx_cross_entropy <- function(logits, targets, reduction = c("mean", "sum", "none")) {
  ...
  one_hot <- mlx_eye(n_classes, dtype = eye_dtype, device = mlx_device(logits))[targets_vec, , drop = FALSE]
  one_hot <- mlx_cast(one_hot, dtype = logits_dtype, device = mlx_device(logits))
