...
#' @noRd
normalize_axes <- function(axes, x) {
  if (is.null(axes)) {
    return(NULL)
  }
  if (!length(axes)) {
  ...
  vapply(axes, normalize_axis_single, integer(1), x = x)
}
...
#' @noRd
normalize_axis_single <- function(axis, x) {
  if (is.na(axis)) {
...
#' @noRd
normalize_axis <- function(axis, x) {
  if (is.null(axis)) {
    return(NULL)
  }
  if (length(axis) != 1L) {
