...
#' @return An mlx array with the requested dtype.
#' @seealso [mlx_dtype()]
#' @export
#' @examples
#' x <- mlx_vector(1:3, dtype = "int32")
#' mlx_cast(x, dtype = "float32")
mlx_cast <- function(x, dtype = NULL) {
  if (!inherits(x, "mlx")) {
  ...
  if (identical(dtype, current_dtype)) {
    return(x)
  }
...
#' @noRd
normalize_new_axes <- function(axes, dims) {
  if (length(axes) == 0L) {
...
mlx_pad <- function(x,
  ...
  target_axes <- if (is.null(axes)) {
    ...
    vapply(axes, function(ax) normalize_axis_single(ax, x) + 1L, integer(1))
  }
...
mlx_moveaxis <- function(x, source, destination) {
  ...
  source_idx <- normalize_axes(source, x)
  dest_idx <- normalize_axes(destination, x)
...
#'
#' @inheritParams mlx_array_required
#' @return An mlx array backed by contiguous storage.
...
mlx_flatten <- function(x, start_axis = 1L, end_axis = NULL) {
  ...
  start_idx <- normalize_axis_single(start_axis, x)
  end_idx <- normalize_axis_single(end_axis, x)
...
#' dim(broadcast)
mlx_broadcast_to <- function(x, shape) {
  x <- as_mlx(x)
...
mlx_where <- function(condition, x, y) {
  ...
  x_dtype <- mlx_dtype(x)
  y_dtype <- mlx_dtype(y)
  result_dtype <- resolve_common_dtype(list(x_dtype, y_dtype, mlx_dtype(condition)))

  condition <- mlx_cast(condition, dtype = "bool")
  x <- mlx_cast(x, dtype = result_dtype)
