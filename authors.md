# Authors and Citation

## Authors

- **David Hugh-Jones**. Author, maintainer.

- **Apple Inc.**. Copyright holder.  
  MLX library downloaded at install time

## Citation

Source:
[`DESCRIPTION`](https://github.com/hughjonesd/Rmlx/blob/v0.2.0/DESCRIPTION)

Hugh-Jones D (2025). *Rmlx: R Interface to MLX Arrays (GPU-Accelerated
with Metal or CUDA)*. R package version 0.1.0.9000,
<https://hughjonesd.github.io/Rmlx/>.

    @Manual{,
      title = {Rmlx: R Interface to MLX Arrays (GPU-Accelerated with Metal or CUDA)},
      author = {David Hugh-Jones},
      year = {2025},
      note = {R package version 0.1.0.9000},
      url = {https://hughjonesd.github.io/Rmlx/},
    }
