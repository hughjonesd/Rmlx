...
test_that("solve works for linear systems", {
  ...
  x_mlx <- solve(A_mlx, b_mlx, device = "cpu")
  x_r <- solve(A, b)
...
test_that("solve works for matrix inverse", {
  ...
  A_inv_mlx <- solve(A_mlx, device = "cpu")
  A_inv_r <- solve(A)
...
test_that("solve works when A is_mlx and b is R matrix", {
  ...
  x_mlx <- solve(A_mlx, b, device = "cpu")
  x_r <- solve(A, b)
...
test_that("chol.mlx respects gpu device and lets MLX report unsupported execution", {
  ...
  spd_gpu <- as_mlx(spd, dtype = "float32")
  expect_error(
    chol(spd_gpu),
    "not yet supported on the GPU",
    fixed = TRUE
...
test_that("CPU-only linalg helpers let MLX report unsupported gpu execution", {
  ...
  mat <- as_mlx(matrix(c(4, 1, 2, 3), 2, 2), dtype = "float32")
  spd <- as_mlx(matrix(c(4, 1, 1, 3), 2, 2), dtype = "float32")
  lower <- as_mlx(matrix(c(2, 1, 0, 3), 2, 2), dtype = "float32")
  chol_factor <- as_mlx(chol(matrix(c(4, 1, 1, 3), 2, 2)), dtype = "float32")
...
test_that("CPU-only linalg helpers respect `device` argument", {
  ...
  expect_no_error(res <- mlx_cholesky_inv(chol_factor, device = "cpu"))
  ...
  expect_no_error(res <- chol(spd, device = "cpu"))
  expect_no_error(res <- chol2inv(chol_factor, device = "cpu"))
  expect_no_error(res <- pinv(mat, device = "cpu"))
})


test_that("CPU-only linalg device argument controls float64 execution", {
  skip_if_not(mlx_has_gpu())

  mat <- with_device("cpu", {
    as_mlx(matrix(c(4, 1, 1, 3), 2, 2), dtype = "float64")
  })
  chol_factor <- with_device("cpu", {
    chol(mat, device = "cpu")
  })

  local_device("gpu")

  expect_error(as.matrix(solve(mat)), "not yet supported on the GPU")
  expect_no_error(res <- solve(mat, device = "cpu"))
  expect_no_error(as.matrix(res))
  expect_equal(mlx_dtype(res), "float64")

  expect_error(as.matrix(chol2inv(chol_factor)), "not yet supported on the GPU")
  expect_no_error(inv <- chol2inv(chol_factor, device = "cpu"))
  expect_no_error(as.matrix(inv))
  expect_equal(mlx_dtype(inv), "float64")
})
...
test_that("svd.mlx with nu=0 and nv=0 returns only singular values", {
  set.seed(404)
  A <- matrix(rnorm(12), 3, 4)
  svd_mlx <- svd(as_mlx(A), nu = 0, nv = 0, device = "cpu")
...
test_that("mlx_inv computes matrix inverse", {
  ...
  A_mlx <- as_mlx(A)
  A_inv_mlx <- mlx_inv(A_mlx, device = "cpu")
  A_inv_r <- solve(A)
...
test_that("mlx_tri_inv computes triangular matrix inverse", {
  ...
  L <- A
  L[upper.tri(L)] <- 0
  L_inv_mlx <- mlx_tri_inv(as_mlx(L), upper = FALSE, device = "cpu")
  L_inv_r <- solve(L)
  ...
  U <- A
  U[lower.tri(U)] <- 0
  U_inv_mlx <- mlx_tri_inv(as_mlx(U), upper = TRUE, device = "cpu")
  U_inv_r <- solve(U)
...
test_that("mlx_cholesky_inv computes inverse via Cholesky", {
  ...
  A_mlx <- as_mlx(A)
  A_inv_mlx <- mlx_cholesky_inv(as_mlx(U), upper = TRUE, device = "cpu")
  A_inv_r <- solve(A)
...
test_that("mlx_cholesky_inv works with lower triangle", {
  ...
  A_inv_mlx <- mlx_cholesky_inv(as_mlx(L), upper = FALSE, device = "cpu")
  A_inv_r <- solve(A)
...
test_that("chol2inv.mlx works like base R chol2inv", {
  ...
  # MLX
  A_mlx <- as_mlx(A)
  U_mlx <- chol(A_mlx, device = "cpu")
  A_inv_mlx <- chol2inv(U_mlx, device = "cpu")
