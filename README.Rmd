...
This package was vibe-coded with Claude/OpenAI Codex.
...
By default, Rmlx searches for a system-installed MLX library. If not found,
configure downloads MLX v0.29.4 from GitHub and builds it locally (typically
5–15 minutes on first install).
...
This will:
1. Search for system MLX in common locations (`/opt/homebrew`, `/usr/local`,
   `/usr/include`, etc.)
2. If not found, download MLX v0.29.4 and build it with optimal backends:
   - **macOS (Apple Silicon):** Metal + CPU backends
   - **Linux with CUDA:** CUDA + CPU backends (requires CUDA toolkit)
   - **Linux without CUDA:** CPU-only backend
...
The configure script will auto-detect it. If it doesn't, specify paths explicitly:
