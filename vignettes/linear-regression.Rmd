...
- A random design matrix `X` of 1000 examples × 100 features
...
We train by repeatedly computing gradients and updating parameters:
...
- **Final loss**: How well our model fits the data
- **Parameter error**: How close our learned weights are to the true weights

## Device Selection

By default, computations run on the best available device. Switch to CPU if needed:
...
- The [getting started vignette](getting-started.html) for basic Rmlx usage
- The [performance benchmarks vignette](performance.html) for timing comparisons
- The [original MLX linear regression example](https://ml-explore.github.io/mlx/build/html/examples/linear_regression.html) in Python

## Notes

- All computations use single-precision floating point (`float32`)
- The gradient computation is entirely automatic—no hand-coded derivatives
- Large computation graphs can accumulate; use `mlx_eval()` to force evaluation
- For more complex models, consider using MLX modules (see the getting started vignette)
