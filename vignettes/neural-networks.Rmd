...
# Loss function
loss_fn <- function(params_list, x, y) {
  # Forward pass
  y_pred <- mlx_forward(mlp, x)
  # Binary cross-entropy loss
  mlx_binary_cross_entropy(y_pred, y)
}
...
params <- mlx_parameters(mlp)

# Training loop
for (epoch in 1:n_epochs) {
  # Compute loss and gradients
  grads <- mlx_grad(loss_fn, params, x_mlx, y_mlx, argnums = 1)

  # Update parameters
  for (i in seq_along(params)) {
    new_value <- params[[i]]$get() - learning_rate * grads[[i]]
    params[[i]]$set(new_value)
    mlx_eval(new_value)  # Force evaluation
  }

  # Print progress
  if (epoch %% 10 == 0) {
    loss <- loss_fn(params, x_mlx, y_mlx)
    cat(sprintf("Epoch %d, Loss: %.4f\n", epoch, as.numeric(as.matrix(loss))))
  }
}
```
...
# Forward pass
mlp$set_training(FALSE)  # Set to evaluation mode (disables dropout)
