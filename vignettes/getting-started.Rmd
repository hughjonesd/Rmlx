...
> **Precision note:** `as_mlx()` always stores data in single precision (`float32`).
> Requesting `dtype = "float64"` will downcast the input with a warning. Use base
> R arrays if you need double precision arithmetic.
...
Remember that computations are always performed in `float32`; the CPU mode is
useful when you need to compare against base R or debug without a GPU.
