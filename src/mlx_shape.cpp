...
SEXP cpp_mlx_meshgrid(SEXP args_, bool sparse, std::string indexing) {
  ...
  std::vector<array> result = meshgrid(arrays, sparse, indexing);
...
SEXP cpp_mlx_broadcast_to(SEXP xp_, Rcpp::IntegerVector shape_) {
  ...
  array result = broadcast_to(arr, shape);
  return make_mlx_xptr(std::move(result));
}
...
SEXP cpp_mlx_broadcast_arrays(SEXP args_) {
  ...
  std::vector<array> result = broadcast_arrays(arrays);
...
SEXP cpp_mlx_pad(SEXP xp_,
  ...
  array result = pad(arr, pad_width, pad_val, mode_str);
  return make_mlx_xptr(std::move(result));
}
...
SEXP cpp_mlx_unflatten(SEXP a_xp_, int axis, IntegerVector shape) {
  ...
  array result = unflatten(arr, ax, new_shape);
  return make_mlx_xptr(std::move(result));
}
...
SEXP cpp_mlx_contiguous(SEXP xp_) {
  ...
  array result = contiguous(arr, /*allow_col_major=*/false);
  return make_mlx_xptr(std::move(result));
}
