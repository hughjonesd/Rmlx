...
// [[Rcpp::export]]
SEXP cpp_mlx_reduce(SEXP xp_, std::string op, int ddof, std::string device_str) {
  MlxArrayWrapper* wrapper = get_mlx_wrapper(xp_);

  array input = wrapper->get();
  StreamOrDevice dev = typed_device(input.dtype(), device_str);
  array result = [&]() -> array {
    if (op == "sum") {
...
// [[Rcpp::export]]
SEXP cpp_mlx_reduce_axis(SEXP xp_,
                         std::string op,
                         int axis,
                         bool keepdims,
                         int ddof,
...
SEXP cpp_mlx_logcumsumexp(SEXP xp_, Rcpp::Nullable<int> axis,
                          bool reverse, bool inclusive, std::string device_str) {
  MlxArrayWrapper* wrapper = get_mlx_wrapper(xp_);
  array arr = wrapper->get();
  StreamOrDevice dev = typed_device(arr.dtype(), device_str);
...
SEXP cpp_mlx_softmax(SEXP xp_, Rcpp::Nullable<Rcpp::IntegerVector> axes,
                     bool precise, std::string device_str) {
  MlxArrayWrapper* wrapper = get_mlx_wrapper(xp_);
  array arr = wrapper->get();
  StreamOrDevice dev = typed_device(arr.dtype(), device_str);
